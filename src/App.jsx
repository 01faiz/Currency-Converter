...
function App() {
  ...
  const [convertCureency, setConvertCureency]=useState(" ");
  ...
  useEffect(()=>{
     async function converCurrency(){
      try {
        const responce = await fetch(`https://api.frankfurter.app/latest?amount=${amount}&from=${toConvert}&to=${fromConvert}`,{signal:controller.signal})
